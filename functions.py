...
def load_and_show(window_name,frame,length,togo,loaded = 0,y = 0,thickness = 60):
    ''' 
    ...
    frame = loading_bar(frame,length,togo,loaded,y,thickness)
    FONT = cv2.FONT_HERSHEY_SIMPLEX
    FONTSCALE = 0.5
    TEXTCOLOUR = (255, 255, 255)
    frame = cv2.putText(frame, "Building your biometric profile. Please wait...", (10,100), FONT, FONTSCALE, TEXTCOLOUR, 1, cv2.LINE_AA, False )
...
def loading_bar(frame,length,togo,loaded = 0,y = 0,thickness = 20):
    ''' 
    displays a loadingbar across the specified length and height this should be called for every frame you want it used
    frame - the frame that it will draw on
    length - the total length it should span
    togo - the total number of jobs to complete
    loaded - the number of these jobs done
    ...
    frame = cv2.rectangle(frame,(0,y),(length,thickness),(255,255,255),5)
    frame = cv2.rectangle(frame,(5,y+5),(processed-5,thickness -5),(0,255,0),-1)
